...
namespace MuslimSurvivalKit.Services
{
    public class Location
    {
        public static IGeolocator Service
        {
            get
            {
        ...
        public static Dictionary<string, (double latitude, double longitude)> CountryDictionary 
        ...
        public static List<string> CountryNames()
        {
            ...
            foreach (var country in CountryDictionary.Keys)
            {
        ...
        public static async Task<(bool success, double latitude, double longitude, string message)> GetCurrentLocation()
        {
            Service.DesiredAccuracy = Data.Settings.DesiredLocationAccuracy;

            try
            {
                //var cached = await Service.GetLastKnownLocationAsync();
                //
                //if (cached != null)
                //    return (true, cached.Latitude, cached.Longitude, "Using cached location");

                var position = await Service.GetPositionAsync(TimeSpan.FromSeconds(30));

                return (true, position.Latitude, position.Longitude, "Using current position");
            }
            catch (Exception exc)
            {
                return (false, 0, 0, exc.Message);
            }
        }

        public static bool IsAvailable()
        {
            return Service.IsGeolocationAvailable;
        }

        public static bool IsEnabled()
        {
            return Service.IsGeolocationEnabled;
        }
    }
}
